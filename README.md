...
- **Queue Management**: Sử dụng **Laravel Horizon** để quản lý và giám sát hàng đợi (Redis).
...
├── docker-compose.yml      # Tệp cấu hình khởi chạy các Docker container (Nginx, PHP, Database, Redis, Horizon)
...
- **Laravel Horizon Dashboard**: Giám sát hàng đợi tại `http://localhost:8000/horizon`.
  - Mặc định cho phép truy cập trong môi trường `local`.
